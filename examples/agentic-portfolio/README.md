...
```
pfrPortfolioReview (classId)
  find ─── AGENT pfrFinderAgent: read-only search tools → ["id", …]
  review ─ SUBPLAN pfrContractsBrief (docIds)                ← also exposed as a chat tool
             each ──── SUBPLAN pfrReviewOne, fan-out over docIds, 8 at a time
             │           read ── DIRECT_TOOL extractDocumentText (no LLM)
             │           facts ─ AGENT pfrFactsAgent → JSON fact sheet (gpt-4o-mini)
             ├ table ─── AGENT pfrTableAgent   ┐
             ├ actNow ── AGENT pfrActNowAgent  ├ three narrow reducers, in parallel (gpt-4o)
             ├ watch ─── AGENT pfrWatchAgent   ┘
             └ brief ─── AGENT pfrBriefAgent: assembles their sections verbatim
```
...
```bash
uxc push --all                                              # 6 prompts, 6 agents, 3 plans, 1 application
...
**45 s** (find 7 s, then review 37 s, ~58 k input / 5.8 k output tokens). Running the 15 fact sheets
8 at a time took 20 s instead of 50 s one by one. From chat,
...
- **Payloads:** prompts read them as `[(${var})]`, raw. `[[${var}]]` would HTML-escape quotes and
  apostrophes.
...
- **One failure fails the batch:** one unreadable document, or one transient provider error, fails
  the whole fan-out and the run. Retry the run.
- **The brief is approximate:** it is LLM-assembled. A near-duplicate contract can be merged in the
  table. Take exact counts and complete lists from `uxc run --plan … --json`, never from the prose.
