...
/** Every task className used by this package's f2.map resources (the connector-jar gate, §F12). */
function pkgTaskClasses(ctx) {
  const classes = new Set();
  let pkg = null;
  try { pkg = ctx.requirePkg(); } catch { return classes; }
  for (const e of pkg.entries('f2.map')) {
    const local = KINDS['f2.map'].readLocal(pkg, e);
    for (const s of local?.obj?.steps ?? []) collectClassNames(s.objectConfiguration, classes);
  }
  return classes;
}
function collectClassNames(oc, into) {
  if (!oc) return;
  if (oc.className) into.add(oc.className);
  for (const f of oc.fields ?? []) {
    if (f.objectConfiguration) collectClassNames(f.objectConfiguration, into);
    for (const item of f.listConfiguration ?? []) collectClassNames(item?.objectConfiguration, into);
  }
}
...
const ROUNDTRIP = [
  ...
  // entries are WRITE-ONLY (echo = lazy ACLProxy, LEARNINGS §37) — compared on the echoed keys
  ['fd.acl', 'ZzUxcDoctorAcl', {}, { writeOnly: ['entries'] }],
  ['ai.prompt', 'zzUxcDoctorPrompt', {}], // camel form — prompts are camel-prefixed
  // uxopian-ai 2026.0.0-ft5 kinds (skipped with a note on older gateways). The gateway checks no
  // references (AI learnings §A13), so dangling probe ids are fine: nothing here is ever RUN.
  ['ai.agent', 'zzUxcDoctorAgent', { objective: 'zzUxcDoctorPrompt' }],
  ['ai.plan', 'zzUxcDoctorPlan', { agent: 'zzUxcDoctorAgent' }],
  ['ai.application', 'zzUxcDoctorApp', {}],
...
export default {
  name: 'doctor',
  summary: 'connectivity gauntlet (--ready: pre-install checklist; --sandbox: GraalVM probe; --f2: fast2 broker; --dups; --roundtrip; --ai-smoke)',
  help: 'uxc doctor [--ready] [--sandbox [--wait s]] [--ai-smoke] [--f2 [--f2-opensearch URL]] [--roundtrip] [--dups]',
  async run(ctx) {
    const report = [];
    let failures = 0;
    ...
    if (ctx.flags.roundtrip) {
      for (const [kind, id, tplFlags, rtOpts] of ROUNDTRIP) {
        const adapter = KINDS[kind];
        let created = false;
        if (typeof adapter.serverSupport === 'function') {
          const reason = await adapter.serverSupport(ctx);
          if (reason) { ctx.out.note(`roundtrip ${kind}: skipped — ${reason}`); continue; }
        }
        try {
          ...
          // write-only fields (accepted by create, never echoed — e.g. ACL entries §37) are
          // excluded from the probe comparison; real syncs preserve them via adapter overlays.
          const strip = (obj) => {
            if (!rtOpts?.writeOnly) return obj;
            const c = { ...obj };
            for (const k of rtOpts.writeOnly) delete c[k];
            return c;
          };
          const hLocal = hashResource(kind, strip(local.obj), Object.values(local.contents ?? {}));
          const hEcho = hashResource(kind, strip(echo.obj), Object.values(echo.contents ?? {}));
          if (hLocal === hEcho) {
            ok(`roundtrip ${kind}`, id + (rtOpts?.writeOnly ? `  (write-only: ${rtOpts.writeOnly.join(',')})` : ''));
            // versioned prompts (ft5): an UPDATE is draft -> publish, not a PUT — prove that leg too
            if (kind === 'ai.prompt' && (await capabilities(ctx, 'uxopian-ai')).caps.promptVersioning) {
              const edited = { obj: { ...local.obj, content: `${local.obj.content} (v1)` } };
              await adapter.update(ctx, id, edited);
              const after = await adapter.readServer(ctx, id);
              if (hashResource(kind, after?.obj) === hashResource(kind, edited.obj)) ok('roundtrip ai.prompt version', `${id} published v1 (draft -> publish)`);
              else bad('roundtrip ai.prompt version', `${id}: the published version does not echo the pushed content`);
            }
          } else {
            const a = canonicalText(kind, strip(local.obj)).split('\n');
            const b = canonicalText(kind, strip(echo.obj)).split('\n');
            let i = 0;
      ...
      ctx.out.note('add --roundtrip for the Zz* push-echo leg (tagclass/tagcategory/documentclass/folderclass/taskclass/acl/prompt; agent/plan/application on uxopian-ai ft5+)');
    }
    ...
    if (ctx.flags.sandbox) {
      ...
      try {
        const extra = ctx.flags.classes ? String(ctx.flags.classes).split(',').map((x) => x.trim()).filter(Boolean) : null;
        const r = await sandboxProbe(ctx, { waitMs, out: ctx.out, classes: extra });
    ...
    // 8b. --f2: the fast2 broker leg. Two of these gates catch failures that are INVISIBLE from
    // the fast2 UI and only surface as a generic 500 at run time (FAST2-LEARNINGS §F10/§F12).
    if (ctx.flags.f2) {
      if (!ctx.clients.f2) {
        bad('f2 surface', 'this target has no fast2 URL — uxc target add <name> … --f2 http://host:1789 --f2-user <email> --f2-password <p>');
      } else {
        ...
        try {
          await f2.login();
          authed = true;
          ok('f2 auth', `${ctx.target.f2User} @ ${f2.base}`);
        } catch (e) { bad('f2 auth', `${e.message}${e.explanation ? ` — ${e.explanation}` : ''}`); }

        if (authed) {
          try {
            const d = await capabilities(ctx, 'fast2');
            ok('dialect fast2', `${d.version ?? '(no version surface)'} -> ${d.dialect ?? '?'} [${d.source}]  caps ${JSON.stringify(d.caps)}`);
          } catch (e) { bad('dialect fast2', e.message); }

          let maps = [];
          try {
            maps = (await f2.get('/api/maps/summary/search-by-pattern?namePattern='))?.collection ?? [];
            const dupes = dupBy(maps, (m) => m.name);
            if (dupes.length) {
              bad('f2 duplicate maps', `${dupes.map(([n, c]) => `${n} x${c}`).join(', ')} — an upload name collision auto-renames to <name>_new1 instead of failing (§F7); delete the extras`);
            } else ok('f2 maps', `${maps.length} map(s), names unique`);
          } catch (e) { bad('f2 maps', e.message); }

          // connector-jar gate: a missing jar means the map deploys fine and FAILS AT RUN TIME
          try {
            // allTask=true is REQUIRED: the default projection lists only top-level TASK classes
            // (~161) and omits the credential/helper beans a map references — e.g.
            // FlowerDocsConnectionProvider — which would read as a missing jar (§F12).
            // The FQCN lives in `name`; `classBaseName` is the simple name.
            const catalog = (await f2.get('/api/catalog?allTask=true')) ?? [];
            const classes = new Set(catalog.map((c) => c.name).filter(Boolean));
            ok('f2 catalog', `${catalog.length} classes (allTask)`);
            const needed = new Set(pkgTaskClasses(ctx));
            for (const req of ctx.pkg?.manifest?.requires?.f2TaskClasses ?? []) needed.add(req);
            const missing = [...needed].filter((c) => !classes.has(c));
            if (needed.size === 0) ctx.out.note('f2 connector jars: no f2.map resources here — nothing to check');
            else if (missing.length) {
              bad('f2 connector jars', `${missing.join(', ')} NOT in the worker catalog — the map will deploy but FAIL AT RUN TIME; install the connector jar into worker-libs/ (or POST /api/workers/upload-library) and restart the worker (§F12)`);
            } else ok('f2 connector jars', `${needed.size} required task class(es) present`);
          } catch (e) { bad('f2 catalog', e.message); }

          // wedged campaigns: unstoppable, undeletable, and they block their map's deletion
          try {
            const camps = (await f2.get('/api/campaigns/search-by-pattern?namePattern=.*'))?.collection ?? [];
            const stuck = [];
            for (const c of camps) {
              const s = String(await f2.get(`/api/campaigns/${encodeURIComponent(c)}/status`)).replace(/^"|"$/g, '');
              if (/^Starting$/i.test(s)) stuck.push(c);
            }
            if (stuck.length) {
              bad('f2 wedged campaigns', `${stuck.join(', ')} stuck in "Starting" — neither stoppable nor deletable via the API, and they BLOCK their map's deletion permanently. Escape: DELETE <opensearch>/f2_campaigns/_doc/<campaign>, drop index f2_<campaign lowercased>, then RESTART the broker (§F8)`);
            } else ok('f2 campaigns', `${camps.length} campaign(s), none wedged`);
          } catch (e) { bad('f2 campaigns', e.message); }

          // the silent killer: OpenSearch refusing index creation fails EVERY campaign start with
          // a generic 500 and wedges the campaign (§F10). Only reachable if the operator gave us
          // the OpenSearch URL — otherwise say how to check it by hand.
          const osUrl = ctx.flags['f2-opensearch'] || process.env.UXC_F2_OPENSEARCH;
          if (osUrl) {
            try {
              const r = await fetch(`${String(osUrl).replace(/\/+$/, '')}/_cluster/settings?flat_settings`, { signal: AbortSignal.timeout(10_000) });
              const j = await r.json();
              const blocked = String(j?.persistent?.['cluster.blocks.create_index'] ?? j?.transient?.['cluster.blocks.create_index'] ?? '') === 'true';
              if (blocked) {
                bad('f2 opensearch create_index', `BLOCKED — every campaign start will fail with a generic 500 and wedge a campaign in "Starting". Clear it: curl -XPUT ${osUrl}/_cluster/settings -H 'Content-Type: application/json' -d '{"persistent":{"cluster.blocks.create_index":null}}' (§F10)`);
              } else ok('f2 opensearch create_index', 'not blocked — campaigns can create their index');
            } catch (e) { bad('f2 opensearch create_index', `${e.message} (checked ${osUrl})`); }
          } else {
            ctx.out.note('f2 opensearch: pass --f2-opensearch http://host:1790 (or UXC_F2_OPENSEARCH) to check the '
              + 'cluster.blocks.create_index flag — when set, EVERY campaign start fails with a generic 500 (§F10)');
          }
        }
      }
    }
