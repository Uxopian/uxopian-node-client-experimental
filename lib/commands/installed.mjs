...
export default {
  ...
  async run(ctx) {
    ...
    if (flags.write) {
      ...
      const res = await writeReceipts(ctx, pkg.manifest, { resources });
      for (const r of res) {
    ...
    out.table(receipts.map((r) => ({
      ...
      tests: r.testsResult ? `${r.testsResult} @ ${String(r.testsPassedAt ?? '').slice(0, 10)}` : '',
    })), [
      { key: 'code' }, { key: 'surface' }, { key: 'version' }, { key: 'uxc' },
      { key: 'installedAt', max: 24 }, { key: 'sha' }, { key: 'tests', max: 28 },
    ]);
