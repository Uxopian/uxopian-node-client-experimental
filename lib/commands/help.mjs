...
fast2 (migration/ingestion maps on a fast2 broker):
  ...
  f2 run <MapName>          start a campaign and report per-step results (--wait --expect-ok)
...
plans (Uxopian AI agentic Plan lifecycle — gateway REST /api/v1/admin/plans):
  plan list                 enumerate Plans
  plan get <id>             read a Plan (node graph + exposeAsTool state)
  plan create <id>          create a Plan (--file plan.json --description --expose-as-tool --tool-description)
  plan update <id>          update an existing Plan (same flags as create)
  plan delete <id>          delete a Plan
  plan run <id>             submit + wait for a terminal status (--payload k=v… --payload-json f --no-wait --timeout ms)
...
kinds: fd.tagclass fd.tagcategory fd.documentclass fd.folderclass fd.taskclass fd.vfclass fd.vfinstance
       ...
       ai.prompt ai.goal ai.mcp ai.llm  f2.map
