...
scopes (FlowerDocs multi-tenant scope lifecycle — SOAP /core/services/scope):
  scope get <id>            read a scope (exists-check + summary)
  scope create <id>         create/update a scope remotely (--blank | --from scope.xml --description --lang --admin)
...
kinds: fd.tagclass fd.tagcategory fd.documentclass fd.taskclass fd.vfclass fd.vfinstance
       fd.workflow fd.acl fd.script fd.guiconfig fd.handler fd.surfacing fd.dataset
       ai.prompt ai.goal ai.mcp ai.llmconf
