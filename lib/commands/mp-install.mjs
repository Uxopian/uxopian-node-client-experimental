...
export default {
  ...
  async run(ctx) {
    ...
    const mpManifest0 = (v) => v?.version?.manifest ?? {};
    ...
    // UPGRADE PRUNING source (DESIGN §23): if a previous version is installed (receipt), fetch
    // ITS catalog — the resources it deployed — so the import can delete what the new version
    // dropped. Best-effort: an old version missing from the marketplace just disables pruning.
    let pruneFrom = null;
    try {
      const code = mpManifest0(vd)?.code;
      const prev = code ? (await readReceipts(ctx, { code })).find((r) => r.version && r.version !== version) : null;
      if (prev) {
        const old = await client.getVersion(slug, prev.version).catch(() => null);
        const objects = old?.version?.catalog?.objects ?? null;
        if (objects) {
          pruneFrom = objects.map((o) => `${o.kind}/${o.id}`);
          out.note(`upgrade from ${prev.version}: ${pruneFrom.length} resource(s) in the installed version's catalog — removals will be pruned (with confirmation)`);
        } else {
          out.warn(`installed ${code}@${prev.version} but its catalog is not on the marketplace — removed resources will NOT be pruned (uxc rm manually)`);
        }
      }
    } catch { /* receipts unreadable -> no pruning */ }

    // dependency pre-check (DESIGN §22) — unmet deps fail BEFORE the download, with the recipe
    await assertDependencies(ctx, mpManifest0(vd), { ignore: !!flags['ignore-dependencies'], out, action: 'install' });
    ...
    try {
      ...
      const res = await importPackage(ctx, file, {
        ...
        ignoreDependencies: !!flags['ignore-dependencies'],
        vars, varFile,
        pruneFrom, yesRemovals: !!flags['yes-removals'], keepRemoved: !!flags['keep-removed'],
      });
