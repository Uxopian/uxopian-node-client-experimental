...
// Special case: fd.handler = registry-less live view (search OperationHandlerRegistration,
// grouped by logical name). Every other kind projects its adapter.list() rows (incl. ai.llm).
...
const PROJECTIONS = {
  ...
  'fd.vfinstance': (o) => ({ id: o.id, classId: o.classId ?? o.data?.classId ?? '', title: o.name ?? title(o) }),
  ...
  'ai.llm': (c) => ({ id: c.id ?? c.provider ?? '', provider: c.provider ?? '', models: (c.llModelConfs ?? []).length }),
  'ai.agent': (a) => ({
    id: a.id ?? '',
    objective: a.objective ?? '',
    tools: a.permissions?.allowAllTools ? 'all' : (a.permissions?.allowedTools ?? []).length || (a.permissions?.allowedToolTags ?? []).join(',') || '',
    secrets: Object.keys(a.secrets ?? {}).length || '',
  }),
  'ai.application': (a) => ({
    id: a.id ?? '',
    provider: a.provider ?? '',
    prompt: a.prompt ?? '',
    'provider/model': a.defaultLlmProvider || a.defaultLlmModel ? `${a.defaultLlmProvider ?? '?'}/${a.defaultLlmModel ?? '?'}` : '',
    tools: a.permissions?.allowAllTools ? 'all' : (a.permissions?.allowedToolTags ?? []).join(','),
  }),
  'ai.tool': (t) => ({ name: t.name ?? '', tags: (t.tags ?? []).join(','), params: (t.params ?? []).length }),
  'ai.plan': (p) => ({
    id: p.id ?? '',
    nodes: (p.nodes ?? []).length,
    types: [...new Set((p.nodes ?? []).map((n) => n.type))].join(','),
    tool: p.exposeAsTool ? 'y' : '',
  }),
};
...
export default {
  ...
  help: 'uxc ls <kind> [--mine] [--fields a,b]   (kinds: uxc help)',
  lock: 'read',
  async run(ctx) {
    ...
    let rows;
    if (kindName === 'fd.handler') rows = await handlerRows(ctx);
    else if (kindName === 'ai.tool') {
      // not a package kind: the gateway's registered native tools — the names agents, plans and
      // applications may whitelist (uxopian-ai ft5+: GET /api/v1/admin/tools)
      rows = ((await ctx.clients.gateway.get('/api/v1/admin/tools')) ?? []).map((t) => ({ id: t.name, ...t }));
    } else {
    ...
    const cols = Object.keys(projected[0] ?? { id: 1 }).map((k) => ({ key: k, max: k === 'id' || k === 'logical' || k === 'name' ? 80 : 60 }));
    ctx.out.table(projected, cols);
    ctx.out.line(`${projected.length} ${kindName}${ctx.flags.mine ? ' (--mine)' : ''} on ${ctx.target.name}`);
    // "0" is a claim about the SERVER, and it used to be a claim the adapter could not back up.
    // If the registry knows of resources of this kind, an empty list is a contradiction worth naming.
    if (!projected.length && kindName !== 'ai.tool') {
      const dir = ctx.flags.dir ?? findPackageDir();
      const known = dir ? openPackage(dir).entries(kindName).filter((e) => !e.retired).length : 0;
      if (known) {
        ctx.out.note(`the registry holds ${known} ${kindName} — an empty server list means they are absent`
          + ' on this target, or that this scope cannot enumerate them: confirm with uxc status --remote');
      }
    }
  },
};
