...
import { lintTagValues, lintPromptVariables, lintAgentic, promptProviderOrder, resourceSizes, sizeWarnings } from '../lint.mjs';
...
export default {
  ...
  async run(ctx) {
    ...
    if (entries.some((e) => ['ai.agent', 'ai.plan', 'ai.application'].includes(e.kind))) {
      for (const f of lintAgentic(pkg)) out.warn(f.message);
    }
