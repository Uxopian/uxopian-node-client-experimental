...
//   offline lints: constrained tag values, include order, prompt variables.
...
import { lintTagValues, lintPromptVariables, lintIncludeOrder, lintAgentic } from '../lint.mjs';
...
export default {
  ...
  async run(ctx) {
    ...
    for (const f of lintPromptVariables(pkg)) {
      checks++;
      if (f.kind === 'unprovided') out.warn(f.message);
