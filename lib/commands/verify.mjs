    ...
    for (const entry of entries) {
      ...
      if (entry.kind === 'fd.handler') {
        if (typeof adapter.liveRegistrations === 'function') {
          checks++;
          let regs = [];
          try { regs = (await adapter.liveRegistrations(ctx, entry.id)) ?? []; }
          catch (e) { failed(`${key}: liveRegistrations failed — ${e.message}`); continue; }
          if (regs.length !== 1) {
            failed(`${key}: expected exactly ONE live registration, found ${regs.length}${regs.length ? ` (${regs.map((r) => regInfo(r).id).join(', ')})` : ''} — push deletes orphans`);
          }
          const reg = regs[0] ? regInfo(regs[0]) : null;
