...
//
//   uxc test [name…] [--target t] [--offline] [--keep] [--json] [--yes] [--list]
//
// - TIERS (BACKLOG #13): a test declaring `offline: true` exercises package logic in-process
//   (t.loadShared + a fake Core) and needs NO server. `--offline` runs only those: no connect, no
//   safety gate, no lock — so they run in parallel with someone else's e2e campaign. Without the
//   flag, offline tests run alongside the live ones as the cheap first tier.
//
...
import { createHarness, createOfflineHarness, checkRequires, makeRunId, TEST_ID_PREFIX } from '../testkit.mjs';
...
export default {
  ...
  help: 'uxc test [name…] [--target t] [--offline] [--keep] [--json] [--yes] [--list]',
  // --offline needs no instance, so it must not queue behind one being written
  lock: (flags) => (flags.offline || flags.list ? 'none' : 'write'),
  async run(ctx) {
    const { flags, out } = ctx;
    ...
    for (const file of files) {
      ...
      const test = {
        file, name: mod?.name ?? file, description: mod?.description ?? '', mod, loadError,
    ...
    // --offline: the tier that needs nothing but this checkout
    const offlineOnly = !!flags.offline;
    if (offlineOnly) {
      // A test whose module does not LOAD cannot say which tier it belongs to (`offline` is read from
      // the module it failed to produce). Filing it with the server-dependent ones hid it: the green
      // count simply dropped by one, with a note nobody reads. It is a failure in every tier.
      const live = selected.filter((t) => !t.offline && !t.loadError);
      selected = selected.filter((t) => t.offline || t.loadError);
      if (!selected.length) {
        fail(`--offline: none of the selected test(s) declare \`offline: true\`${live.length ? ` (${live.length} need a server)` : ''}`
          + ' — add `offline: true` to a test that exercises package logic in-process (t.loadShared + a fake Core).');
      }
      if (live.length) out.note(`--offline: ${live.length} server-dependent test(s) skipped`);
    }
    ...
    // ---- safety gate, then connect (only if a live test will actually run) ----
    const needsServer = selected.some((t) => !t.offline && !t.loadError);
    if (needsServer) {
      ctx.connect();
    ...
    out.line(needsServer
      ? `running ${selected.length} test(s) on ${ctx.target.name} (scope ${ctx.target.scope}) — run ${runId}, fixtures ${TEST_ID_PREFIX}_*_${runId}tNN`
      : `running ${selected.length} offline test(s) — no server, no lock`);
    ...
    for (const test of selected) {
      ...
      // `requires` describes the SERVER an offline test does not use — checking it would make the
      // cheap tier depend on the thing it exists to avoid
      let gate;
      if (test.offline) gate = { ok: true };
      else {
      ...
      // per-test run suffix: two tests minting the same hint (e.g. both create a CtContract)
      // must NEVER share a fixture id — a kept/undeleted fixture would corrupt the later test
      const testRunId = `${runId}t${String(rows.length).padStart(2, '0')}`;
      const { t, teardown } = test.offline
        ? createOfflineHarness(ctx, { testsDir, log: (m) => out.note(`· ${m}`) })
        : createHarness(ctx, { runId: testRunId, testsDir, log: (m) => out.note(`· ${m}`) });
      ...
      try {
        ...
        row.status = e?.testSkip ? 'skip' : 'fail';
      ...
      row.ms = Date.now() - t0;
      out.line(row.status === 'pass' ? `✓ ${test.name} (${Math.round(row.ms / 1000)}s)`
        : row.status === 'skip' ? `○ ${test.name} — SKIP: ${row.detail}`
        : `✗ ${test.name} — ${row.detail}`);
    }
    ...
    out.line(`${passed} pass · ${failed} fail · ${skipped} skip${needsServer ? '' : ' (offline tier — proves logic, not a deploy)'}`);

    let stamped = null;
    if (failed === 0 && passed > 0 && needsServer) {
      try {
    ...
    out.result({ target: ctx.target?.name ?? null, offline: !needsServer, runId, tests: rows, passed, failed, skipped, stamped });
  },
};
