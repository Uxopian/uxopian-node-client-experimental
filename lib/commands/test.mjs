...
export default {
  ...
  async run(ctx) {
    ...
    out.line(`running ${selected.length} test(s) on ${ctx.target.name} (scope ${ctx.target.scope}) — run ${runId}, fixtures ${TEST_ID_PREFIX}_*_${runId}tNN`);
    ...
    for (const test of selected) {
      ...
      const { t, teardown } = createHarness(ctx, { runId: testRunId, testsDir, log: (m) => out.note(`· ${m}`) });
