...
export default {
  ...
  help: 'uxc import <pkg.uxpkg|dir> [--code-remap old=new] [--force] [--expect-sha256 <hash>]',
  async run(ctx) {
    const { flags, out } = ctx;
    const args = [...ctx.args];
    const force = reclaim(flags, args, 'force');
    const src = args[0];
    if (!src) fail('usage: uxc import <pkg.uxpkg|dir> [--code-remap old=new] [--force] [--expect-sha256 <hash>]');
    ...
    const res = await importPackage(ctx, src, { remap, force, expectSha256 });
