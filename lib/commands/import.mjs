...
export default {
  ...
  async run(ctx) {
    ...
    const ignoreDependencies = reclaim(flags, args, 'ignore-dependencies');
    const src = args[0];
    ...
    const res = await importPackage(ctx, src, { remap, force, expectSha256, ignoreClientVersion, ignoreServerVersion, ignoreDependencies, vars, varFile, yesRemovals: reclaim(flags, args, 'yes-removals'), keepRemoved: reclaim(flags, args, 'keep-removed') });
