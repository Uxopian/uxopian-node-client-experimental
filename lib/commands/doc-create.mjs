...
export default {
  name: 'doc create',
  summary: 'create a document (--tag k=v… --file path --id --name --acl)',
  help: 'uxc doc create <classId> [--tag k=v]… [--file path] [--id id] [--name name] [--acl aclId]',
  async run(ctx) {
    ...
    const body = {
      ...
      category: 'DOCUMENT',
      data: {
        classId, owner: ctx.target.user, creationDate: fdTimestamp(), lastUpdateDate: fdTimestamp(),
        // --acl: infra docs (e.g. Route/Gateway) need a NON-default ACL like acl-admin
        ...(ctx.flags.acl ? { ACL: String(ctx.flags.acl) } : {}),
      },
      tags,
    };
