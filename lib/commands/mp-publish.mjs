...
import { includeDirectiveFiles, INCLUDE_MIN_CLIENT } from '../include.mjs';
...
export default {
  ...
  async run(ctx) {
    ...
    // @include lint: a pre-0.12 client pushes the directive line VERBATIM (a silently broken
    // script on the server) — any script/handler source using @include must pin
    // minClientVersion >= 0.12.0 so old clients refuse the package instead.
    const includeFiles = includeDirectiveFiles(pkg.dir);
    if (includeFiles.length && (!minClient || compareSemver(minClient, INCLUDE_MIN_CLIENT) < 0)) {
      errors.push(`@include directives in ${includeFiles.slice(0, 3).join(', ')}${includeFiles.length > 3 ? ` (+${includeFiles.length - 3} more)` : ''} require "minClientVersion": "${INCLUDE_MIN_CLIENT}" — older clients push the directive line verbatim (silently broken script)`);
    }
