...
const FLAGS = {
  ...
  diff: ['--base', '--full'],
  pull: ['--all'],
  push: ['--changed', '--all', '--force', '--settle', '--recreate', '--revive'],
  ...
  import: ['--code-remap', '--force', '--expect-sha256'],
  ls: ['--mine', '--fields'],
...
/** All kind tokens offered for completion (real adapters + the inspect-only ai.llmconf). */
export function kindList() {
  return [...new Set([...Object.keys(KINDS), 'ai.llmconf'])].sort();
}
