...
//       1. operator override — targets.json `fdVersion` / `aiVersion` / `f2Version` (env
//          UXC_FD_VERSION / UXC_AI_VERSION / UXC_F2_VERSION): pins the dialect when detection is
//          impossible or wrong;
//       2. a version endpoint — FlowerDocs Core: GET /core/actuator/info -> {version:"2026.0.0"}
//          (verified live, LEARNINGS §25); fast2 broker: GET /actuator/info -> {build:{version}}
//          (FAST2-LEARNINGS §F2); uxopian-ai exposes NO version (actuator/info is still {} on
//          2026.0.0-ft5 — AI learnings §A11);
//       3. capability FINGERPRINT — uxopian-ai: GET /api/v1/admin/prompts answers 200-array on
//          2026-07+ builds and 500'd on 2025-era gateways (§8/§17/§25); on 2026.0.0-ft5 every row
//          of that same list carries its served `version` number (prompt versioning, §A11) — an
//          empty list falls back to GET /api/v1/admin/plans, which shipped in the same release.
//   - Version strings are the PRODUCT versions (uxopian-ai 2026.0.0-ft4, 2026.0.0-ft5 …).
//     Prerelease identifiers compare NATURALLY here (ft10 > ft5), unlike plain semver.
...
export const DIALECTS = {
  ...
  'uxopian-ai': {
    ...
    ranges: [
      ...
      { name: 'ai-2025', max: '2026.0.0-ft1', caps: {
        adminPromptList: false, promptVersioning: false, promptWrite: 'admin-v1', goals: true, agenticPlans: false, applications: false,
      } },
      // 2026-07 build (2026.0.0-ft4): admin prompt list works and returns FULL objects
      // (role/provider/model…); prompts are still flat records written with PUT (id in body).
      { name: 'ai-2026-07', max: '2026.0.0-ft5', caps: {
        adminPromptList: true, promptVersioning: false, promptWrite: 'admin-v1', goals: true, agenticPlans: false, applications: false,
      } },
      // 2026.0.0-ft5 (AI learnings §A11-§A14): prompts are version histories (draft -> publish,
      // the bare PUT /admin/prompts is GONE — it answers 500), goals are REMOVED (404, and a
      // GOAL request content is a 400), the Agentic Plan engine adds agents + plans, and
      // Applications scope provider/model/tools per calling surface (X-Application-Id).
      { name: 'ai-2026-ft5', max: null, caps: {
        adminPromptList: true, promptVersioning: true, promptWrite: 'versioned-v1', goals: false, agenticPlans: true, applications: true,
      } },
    ],
  },
  fast2: {
    oldestSupported: '2025.0',
    ranges: [
      // fast2 2026: REST under /api, pure-JSON map CRUD, a REAL version surface at
      // /actuator/info, and the upload endpoint that auto-renames on collision instead of
      // erroring (so pushes go through POST/PUT, never upload) — FAST2-LEARNINGS §F1/§F2/§F5/§F7.
      { name: 'f2-2026', max: null, caps: {
        apiPrefix: '/api', actuatorInfo: true, mapJsonCrud: true, uploadAutoRenames: true,
      } },
    ],
  },
};

/** The targets.json field that pins each product's dialect (detection override). */
const PIN_FIELD = { flowerdocs: 'fdVersion', 'uxopian-ai': 'aiVersion', fast2: 'f2Version' };

/**
 * Split letter+digit prerelease identifiers so they compare numerically: '2026.0.0-ft10' ->
 * '2026.0.0-ft.10'. Plain semver ranks 'ft10' BELOW 'ft5' (ASCII order), which would drop a
 * future uxopian-ai build into an older dialect.
 */
export function naturalVersion(v) {
  const s = String(v ?? '');
  const plus = s.indexOf('+');
  const core = plus === -1 ? s : s.slice(0, plus);
  const dash = core.indexOf('-');
  if (dash === -1) return s;
  const pre = core.slice(dash + 1).split('.')
    .flatMap((id) => { const m = /^([A-Za-z]+)(\d+)$/.exec(id); return m ? [m[1], m[2]] : [id]; })
    .join('.');
  return `${core.slice(0, dash + 1)}${pre}${plus === -1 ? '' : s.slice(plus)}`;
}
const cmpVersion = (a, b) => compareSemver(naturalVersion(a), naturalVersion(b));
...
export function rangeForVersion(product, version) {
  ...
  if (p.oldestSupported && cmpVersion(version, p.oldestSupported) < 0) {
    throw new Error(
  ...
  for (const r of p.ranges) {
    if (r.max == null || cmpVersion(version, r.max) < 0) return r;
  }
...
async function detectUxopianAi(ctx) {
  ...
  let promptVersioning = false;
  if (Array.isArray(admin) && admin.length) {
    // 2026.0.0-ft5: each row is the prompt's SERVED version and carries its number (§A11)
    promptVersioning = admin.some((p) => Number.isInteger(p?.version));
  } else if (Array.isArray(admin)) {
    // an empty list proves nothing — ask for the Agentic Plan surface of the same release
    let plans = null;
    try { plans = await ctx.clients.gateway.tryGet('/api/v1/admin/plans'); } catch { plans = null; }
    promptVersioning = Array.isArray(plans);
  }
  return { version: null, source: 'probe', fingerprint: { adminPromptList: Array.isArray(admin), promptVersioning } };
}

/** fast2 HAS a version surface: GET /actuator/info -> {build:{version}} (FAST2-LEARNINGS §F2). */
async function detectFast2(ctx) {
  const override = process.env.UXC_F2_VERSION || ctx.target?.f2Version;
  if (override) return { version: String(override), source: 'override' };
  if (!ctx.clients?.f2) {
    throw new Error(
      'no fast2 surface on this target — add it: uxc target add <name> … --f2 http://host:1789 --f2-user <email> --f2-password <p>',
    );
  }
  try {
    const r = await ctx.clients.f2.raw('GET', '/actuator/info');
    const v = r.status < 400 ? r.json?.build?.version : null;
    if (v) return { version: String(v), source: 'actuator', build: r.json?.build?.artifact };
  } catch { /* actuator unreachable */ }
  return { version: null, source: 'unknown' };
}
...
export async function capabilities(ctx, product) {
  ...
  const detect = product === 'flowerdocs' ? detectFlowerdocs
    : product === 'uxopian-ai' ? detectUxopianAi
    : product === 'fast2' ? detectFast2 : null;
  ...
  if (d.version) {
    ...
    if (range) {
      (ctx.out?.warn ?? console.error)(
        `${product}: server version undetectable — assuming the newest known dialect (${range.name}); pin it via targets.json ${PIN_FIELD[product] ?? 'aiVersion'} if wrong`,
      );
    }
  }
...
export async function assertServerSupported(ctx, manifest, { ignore = false, out, action = 'deploy' } = {}) {
  ...
  for (const [product, patterns] of Object.entries(declared)) {
    ...
    if (!d.version) {
      out?.warn?.(`${product}: supportedVersions ${JSON.stringify(patterns)} declared but the server version is undetectable — pattern not enforced (pin ${PIN_FIELD[product] ?? 'aiVersion'} on the target to enforce)`);
