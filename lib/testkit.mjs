...
export function createHarness(ctx, { runId, testsDir, log = () => {} }) {
  ...
  const t = {
    ...
    id: (hint) => mintId(pkg.manifest?.code ?? 'PKG', hint, runId),
    ...
    doc: {
       ...
       * { classId, name?, id?, tags?, data?, acl?, file? }
       ...
       */
      async create({ classId, name, id, tags = {}, data = {}, acl, file } = {}) {
        ...
        if (file) {
          const f = typeof file === 'string'
            ...
            : file;
          files.push({ bytes: f.bytes, filename: f.filename ?? 'fixture.bin', mime: f.mime ?? 'application/octet-stream' });
        }
        const body = {
          id: docId,
          name: name ?? docId,
