...
const KIND_LABEL = {
  ...
  'ai.prompt': 'AI prompt', 'ai.goal': 'AI goal', 'ai.mcp': 'MCP config', 'ai.llm': 'LLM provider',
  'ai.agent': 'AI agent', 'ai.plan': 'AI plan', 'ai.application': 'AI application',
};
...
function noteFor(kind, obj) {
  switch (kind) {
    ...
    case 'fd.folderclass': {
      ...
      const refs = obj.tagReferences ?? [];
      return [Array.isArray(kids) && kids.length ? `${kids.length} child rule${kids.length > 1 ? 's' : ''}` : 'folder class',
        Array.isArray(refs) && refs.length ? `${refs.length} tag refs` : null].filter(Boolean).join(' · ');
    }
    case 'fd.taskclass': {
    ...
    case 'fd.workflow': {
      const steps = obj.taskClasses ?? [];
      return Array.isArray(steps) && steps.length ? `${steps.length} steps` : 'workflow';
    }
    case 'fd.acl': {
      const entries = obj.entries ?? [];
      return Array.isArray(entries) && entries.length ? `${entries.length} entries` : 'ACL';
    }
    case 'fd.handler': {
    ...
    case 'ai.llm': {
      const models = obj.llModelConfs ?? [];
      return [obj.provider, Array.isArray(models) && models.length ? `${models.length} models` : null]
        .filter(Boolean).join(' · ') || 'LLM provider';
    }
    case 'ai.agent': return obj.objective ? `runs ${obj.objective}` : 'agent';
    case 'ai.application': return [obj.provider, obj.defaultLlmProvider && obj.defaultLlmModel ? `${obj.defaultLlmProvider}/${obj.defaultLlmModel}` : null]
      .filter(Boolean).join(' · ') || 'application';
    case 'ai.plan': {
      const nodes = Array.isArray(obj.nodes) ? obj.nodes : [];
      return [`${nodes.length} node${nodes.length === 1 ? '' : 's'}`, obj.exposeAsTool ? 'exposed as a tool' : null]
        .filter(Boolean).join(' · ');
    }
