...
import { pruneRemoved } from './prune.mjs';
...
export async function importPackage(ctx, src, { remap = null, force = false, expectSha256 = null, ignoreClientVersion = false, ignoreServerVersion = false, ignoreDependencies = false, vars = {}, varFile = {}, pruneFrom = null, yesRemovals = false, keepRemoved = false } = {}) {
  ...
  // no longer carries are deleted after a printed list + confirmation. `pruneFrom` = the old
  // resource keys (mp install supplies them from the installed version's marketplace catalog).
  if (pruneFrom?.length) {
    await pruneRemoved(ctx2, pruneFrom, pkg.entries(), {
  ...
  const receipts = await writeReceipts(ctx2, pkg.manifest, { artifactSha, variables: appliedVars ?? undefined });
