...
export const KIND_FORM = {
  ...
  'ai.mcp': 'camel',
  'ai.agent': 'camel',      // ids double as tool names when a plan is exposed as a tool
  'ai.plan': 'camel',
  'ai.application': 'camel', // the id IS the name (the gateway derives one from the other)
  'ai.llm': 'verbatim',     // global provider ids (openai / mistral-ai) — never project-prefixed
  'f2.map': 'pascal',       // fast2 map NAME is the key (CtIngestFromShare); '_' is legal in it
};
...
export function conventionalId(kind, manifest, name) {
  ...
  if (form === 'verbatim') return name; // global ids (LLM providers) are never project-prefixed
  ...
  // cross-form guard: a name confidently carrying ANOTHER form's prefix is re-formed,
  // never double-prefixed ('CtFoo' for ai.prompt -> 'ctFoo', not 'ctCtFoo')
  const bare = stripPrefixForm(forms, name) ?? name;
  if (form === 'kebab') return prefix + bare.replace(/^[-_]+/, '').replace(/_/g, '-').toLowerCase();
  if (form === 'camel') return prefix + bare.charAt(0).toUpperCase() + bare.slice(1);
  return prefix + bare.charAt(0).toUpperCase() + bare.slice(1); // pascal
}

/** Strip a CONFIDENT prefix form off a name (upper 'CT_', kebab 'ct-', or pascal/camel followed
 *  by an uppercase letter — the same heuristic as looksOwned). Null when nothing confident. */
function stripPrefixForm(f, name) {
  if (name.startsWith(f.upper)) return name.slice(f.upper.length);
  if (name.toLowerCase().startsWith(f.kebab)) return name.slice(f.kebab.length);
  if (name.startsWith(f.pascal) && /^[A-Z]/.test(name.slice(f.pascal.length))) return name.slice(f.pascal.length);
  if (name.startsWith(f.camel) && /^[A-Z]/.test(name.slice(f.camel.length))) return name.slice(f.camel.length);
  return null;
}
