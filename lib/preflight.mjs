...
export async function readinessChecks(ctx, { pkg = null } = {}) {
  ...
  if (pkg) {
    ...
    // dependency rows (DESIGN §22): what this package NEEDS on the target, vs the receipts
    for (const d of await checkDependencies(ctx, pkg.manifest)) {
      add('L3.package', `dependency ${d.code}`, d.ok,
        d.ok ? `${d.installed ? `installed ${d.installed}` : d.why}` : `${d.why}`,
        d.ok ? undefined : `uxc mp install ${d.slug ?? d.code} (then re-run)`);
    }
  }
  return rows;
}
 ...
 */
export async function sandboxProbe(ctx, { waitMs = 120_000, pollMs = 5_000, out, classes } = {}) {
  ...
  try {
    ...
    await pushContentDoc(ctx, {
      ...
      files: [{ bytes: Buffer.from(probeScript(classes && classes.length ? classes : PROBE.classes)), filename: 'handler.js', mime: 'application/javascript' }],
    });
