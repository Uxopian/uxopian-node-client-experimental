...
/** Drop full-line `//` comments and collapse blank-line runs. uxc:include markers are kept (they
 *  delimit blocks for pull's isExpansionOf and for a human reading the server copy). */
export function stripFullLineComments(text) {
  const out = [];
  let blank = 0;
  for (const line of String(text).split('\n')) {
    if (/^[ \t]*\/\//.test(line) && !/^[ \t]*\/\/ (>>>|<<<) uxc:include /.test(line)) continue;
