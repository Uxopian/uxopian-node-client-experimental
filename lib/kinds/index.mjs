...
//   async serverSupport(ctx)                 -> null | 'reason'  OPTIONAL dialect gate: a reason when the
//                                               connected server has no such concept (ai.goal on
//                                               uxopian-ai ft5+, ai.agent/ai.plan/ai.application before it) — the sync
//                                               engine then reports the resource 'unsupported'
//                                               instead of failing the run (DESIGN §18)
// }
//
...
import aiLlm from './ai-llm.mjs';
import aiAgent from './ai-agent.mjs';
import aiPlan from './ai-plan.mjs';
import aiApplication from './ai-application.mjs';
import f2Map from './f2-map.mjs';
...
export const PUSH_ORDER = [
  ...
  'fd.surfacing', 'ai.llm', 'ai.prompt', 'ai.goal', 'ai.mcp',
  // agentic kinds after everything they name: an agent runs a prompt and may use MCP servers; a
  // plan runs agents (and sub-plans); an application names a prompt and may grant sub-plans.
  // The gateway checks none of it (AI learnings §A13/§A15).
  'ai.agent', 'ai.plan', 'ai.application',
  // fast2 LAST: a map injects into FlowerDocs classes and calls AI prompts by id, so everything it
  // references must already exist. Delete runs in reverse, so the map goes first on teardown.
  'f2.map',
];
...
export const KINDS = Object.fromEntries(
  [tagclass, tagcategory, documentclass, taskclass, folderclass, vfclass, vfinstance, workflow, acl,
   script, guiconfig, handler, surfacing, dataset, aiPrompt, aiGoal, aiMcp, aiLlm, aiAgent, aiPlan, aiApplication, f2Map]
    .map((a) => [a.kind, a]),
);
