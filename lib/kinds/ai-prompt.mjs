...
// list. Writes go through the admin endpoints (POST, 409 -> PUT with id in body).
//
 ...
 * A strategy = { shape(body) -> body, create(ctx, body), update(ctx, id, body) }.
 * The adapter keeps the strategy-INDEPENDENT safety: exists-check-first + the post-create
 * duplicate assertion.
 */
const WRITE_STRATEGIES = {
  // 2025-era AND 2026-07 gateways: POST /admin/prompts (409 -> PUT), PUT with id in BODY.
  'admin-v1': {
    shape: (body) => body,
  ...
  // when the announced prompt-versioning / working-copy release lands, add e.g. 'working-copy-v1'
  // here (create version -> publish; shape() adding/dropping the fields that release requires)
  // and point a new dialect range's caps.promptWrite at it.
};
...
const adapter = {
  ...
  async create(ctx, local) {
    ...
    await strategy.create(ctx, strategy.shape(local.obj));
    invalidate(ctx);
  ...
  async update(ctx, id, local) {
    await maybeLintHelpers(ctx, local);
    const strategy = await writeStrategy(ctx);
    await strategy.update(ctx, id, strategy.shape(local.obj));
    invalidate(ctx);
  },
