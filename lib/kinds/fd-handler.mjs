 ...
 * -> { live: id|null, n: maxN|null, orphans: [ids], recovered: [ids] }
 *
 * SEARCH IS EVENTUALLY CONSISTENT (LEARNINGS §25 — verified live: a just-created doc GETs fine
 * while search returns found=0; a rebuilt/blank index behaves the same for much longer). A search
 * that misses the live registration made the rotation deploy _v1 NEXT TO an invisible _v13 —
 * TWO live registrations firing per event = duplicated downstream objects. So search results are
 * never trusted alone: `hints` (candidate _vN ids from sync state) missing from the search are
 * verified by DIRECT GET and merged in (`recovered`). Callers pass state.deployedId as a hint.
 * Residual risk: a fresh clone (no state) against a blank index cannot see an invisible live
 * registration — run `uxc doctor --dups` after any index rebuild before pushing handlers.
 */
export async function liveRegistrations(ctx, logical, { hints = [] } = {}) {
  ...
  const seen = new Set(matches.map((x) => x.id));
  const recovered = [];
  for (const hint of hints.filter(Boolean)) {
    const m = re.exec(hint);
    if (!m || seen.has(hint)) continue;
    seen.add(hint); // never GET the same hint twice
    if (await ctx.clients.core.getDoc(hint)) {
      matches.push({ id: hint, n: Number(m[1]) });
      recovered.push(hint);
    }
  }
  if (recovered.length) {
    (ctx.out?.warn ?? console.error)(
      `${KIND}/${logical}: search index is LAGGING — ${recovered.length} live registration(s) recovered by direct GET (${recovered.join(', ')}); rotation will account for them`,
    );
  }
  if (!matches.length) return { live: null, n: null, orphans: [], recovered };
  matches.sort((a, b) => b.n - a.n);
  return { live: matches[0].id, n: matches[0].n, orphans: matches.slice(1).map((x) => x.id), recovered };
}

/** The state-recorded deployed id for a logical name — the hint that heals search lag. */
function stateHint(ctx, logical) {
  try {
    return ctx.pkg && ctx.target?.name
      ? ctx.pkg.resState(ctx.target.name, KIND, logical)?.deployedId ?? null
      : null;
  } catch { return null; }
}
...
async function push(ctx, logicalOrEntry, local) {
  ...
  const reg = await liveRegistrations(ctx, logical, { hints: [stateHint(ctx, logical)] });
  const n = (reg.n ?? 0) + 1;
...
export async function setEnabled(ctx, logicalOrId, enabled) {
  ...
  const { live } = await liveRegistrations(ctx, logical, { hints: [stateHint(ctx, logical)] });
...
const adapter = {
  ...
  async extras(ctx, id) {
    const logical = splitHandlerId(id).logical;
    const { live, n, orphans } = await liveRegistrations(ctx, logical, { hints: [stateHint(ctx, logical)] });
  ...
  readLocal(pkg, entry) {
    ...
    if (existsSync(sp)) contents[basename(scriptRel)] = expandIncludes(readFileSync(sp), resolve(sp), resolve(pkg.dir));
    if (obj.filter) {
  ...
  writeLocal(pkg, entry, { obj, contents = {} }) {
    ...
    for (const [key, bytes] of Object.entries(contents)) {
      ...
      // @include pull-guard: the server holds the EXPANDED script; when the local source still
      // expands to exactly these bytes, writing would only flatten the directive — skip.
      if (isExpansionOf(bytes, resolve(target), resolve(pkg.dir))) continue;
