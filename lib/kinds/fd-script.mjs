...
// document WITHOUT any RegistrationOrder tag — the GUI never loads it. That is the shape of a
// library a handler fetches and `load()`s at run time (FLOWERDOCS-LEARNINGS §40): composed once
// by @include, paid once, instead of once per handler. A MISSING key stays a validation error (a
// browser script that forgot its order is a silent no-op, not a library).
