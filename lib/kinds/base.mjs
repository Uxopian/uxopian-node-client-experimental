  ...
  const adapter = {
    ...
    async create(ctx, { obj }) {
      ...
      try {
        ...
        // F00903/T00108: the object appeared between the push engine's server-check and this
        // create (TOCTOU) — HEAL by updating in place, but only where update is allowed:
        // managed kinds and createOnly+inPlaceUpdate (fd.taskclass). Pure createOnly kinds
        // (fd.vfinstance) rethrow — silently updating them would bypass their policy.
        if (!isExistsError(e) || !(defaultPolicy === 'managed' || inPlaceUpdate)) throw e;
        (ctx.out?.warn ?? console.error)(`${kind}/${obj.id}: created concurrently elsewhere — healed by updating in place`);
        await adapter.update(ctx, obj.id, { obj });
      }
    },
