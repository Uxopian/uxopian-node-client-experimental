...
function canonRow(src, classId) {
  ...
  // FD-2026 canonicalization drops empty arrays at ANY depth — a tag-less row has NO row.tags
  if (Array.isArray(row.tags)) row.tags.sort((a, b) => strCmp(a.name, b.name));
 ...
 *  SEARCH IS EVENTUALLY CONSISTENT (LEARNINGS §25): a lagging/rebuilt index hides existing docs,
 *  which used to make pull DROP local rows as "deleted on server" and push misclassify rows.
 *  `known` ids (from sync state + the local JSONL) missing from the search are verified by
 *  DIRECT GET and merged back in — decisions then run on recovered truth, not index luck. */
export async function readServerRows(ctx, ds, { known = [] } = {}) {
  ...
  const missing = [...new Set(known.map(String))].filter((id) => !rows.has(id)).sort(strCmp);
  let recovered = 0;
  for (const id of missing) {
    const doc = await core.getDoc(id);
    if (doc && String(doc.data?.classId ?? ds.classId) === String(ds.classId)) {
      rows.set(String(doc.id), canonRow(doc, ds.classId));
      recovered++;
    }
  }
  if (recovered) {
    (ctx.out?.warn ?? console.error)(
      `fd.dataset(${ds.classId}): search index is LAGGING — ${recovered}/${missing.length} known row(s) invisible to search were recovered by direct GET; pull/push decisions use the recovered truth`,
    );
  }
  return rows;
}

/** Known row ids for lag recovery: sync-state rows + local non-tombstone rows. */
function knownRowIds(ctx, pkg, entry, localRead) {
  const state = Object.keys(pkg?.resState?.(ctx.target?.name, KIND, entry.id)?.rows ?? {});
  const local = [...(localRead?.rows?.keys() ?? [])].filter((id) => !isTombstone(localRead.rows.get(id)));
  return [...new Set([...state, ...local])];
}
...
async function load(ctx, pkg, idOrEntry) {
  ...
  const server = await readServerRows(ctx, ds, { known: knownRowIds(ctx, pkg, entry, localRead) });
...
const adapter = {
  ...
  async get(ctx, id) {
    ...
    const entry = entryOf(pkg, id);
    const rows = await readServerRows(ctx, ds, { known: knownRowIds(ctx, pkg, entry, adapter.readLocal(pkg, entry)) });
  ...
  async remove(ctx, id) {
    ...
    const ds = requireDataSet(pkg, id);
    const entry = entryOf(pkg, id);
    // include known ids: a lagging index must not shield rows from an explicit delete-all
    const rows = await readServerRows(ctx, ds, { known: knownRowIds(ctx, pkg, entry, adapter.readLocal(pkg, entry)) });
  ...
  template(ctx, name, flags = {}) {
    ...
    // `uxc add` writes the manifest dataSets entry itself (BACKLOG #21) — printing "the manifest
    // entry must exist" next to "created fd.dataset/…" left the caller unsure what had happened.
    // A hand-made dataset with no binding is still caught, by validate().
