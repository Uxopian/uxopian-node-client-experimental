 ...
 */
export function resolveMasks(obj, live, id, kind = 'ai.mcp') {
  const missing = [];
  ...
  if (missing.length) {
    throw new Error(`${kind}/${id}: masked secret(s) at [${missing.join(', ')}] have no live server value — put the real secret in the local file before pushing`);
  }
  return out;
}
