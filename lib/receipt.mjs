...
export const FD_TAGS = ['UxcPackageCode', 'UxcPackageVersion', 'UxcClientVersion', 'UxcInstalledAt', 'UxcArtifactSha'];
...
export function buildReceipt(manifest, { artifactSha = null, when = nowIso(), variables = null } = {}) {
  return {
...
export async function ensureFdInfra(ctx) {
  ...
  if (!(await core.getOne(`/rest/documentclass/${encodeURIComponent(FD_CLASS)}`))) {
...
export function receiptFromAiPrompt(p) {
  ...
  try {
    ...
    return { surface: 'uxopian-ai', code: r.code, version: r.version, uxcVersion: r.uxcVersion, installedAt: r.installedAt, artifactSha: r.artifactSha ?? null };
  } catch { return null; }
}
